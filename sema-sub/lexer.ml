...
let rec read (lexbuf : Sedlexing.lexbuf) =
  ...
  | "true" -> make lexbuf (fun i -> KTRUE i)
  | "false" -> make lexbuf (fun i -> KFALSE i)
  | "bool" -> make lexbuf (fun i -> TBOOL i)
  | ":type-norm" -> make lexbuf (fun i -> CTYPENORM i)
  | ":type" -> make lexbuf (fun i -> CTYPE i)
  | "<:" -> make lexbuf (fun i -> SUBTYPE i)
  | "~=" -> make lexbuf (fun i -> ISOTYPE i)
