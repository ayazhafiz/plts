...
type ty =
  ...
  | TyFloat
  | TyNat
  (* A universally qualified type. *)
  | TyAll of string * ty
  (* An existentionally qualified type. *)
  | TySome of string * ty
...
type term =
  ...
  | TmTyAbs of info * string * term
  (* { *X, ty } as SomeTy *)
  | TmPack of info * ty * term * ty
  (* let { X, t1 } = SomeTy in t2 *)
...
let tmInfo t =
  ...
  | TmLoc (info, _) -> info
  | TmTyAbs (info, _, _) -> info
  | TmTyApp (info, _, _) -> info
  | TmPack (info, _, _, _) -> info
  | TmUnpack (info, _, _, _, _) -> info
...
let tyMap ontype cutoff ty =
  let rec walk cutoff ty =
    ...
    | TyNat -> TyNat
    | TyBool -> TyBool
    | TyAll (name, tyT) -> TyAll (name, walk (cutoff + 1) tyT)
    | TySome (name, tyT) -> TySome (name, walk (cutoff + 1) tyT)
  in
  walk cutoff ty
...
let tmMap onvar ontype cutoff term =
  let rec walk cutoff term =
    ...
    | TmTyAbs (info, tyName, term) ->
        TmTyAbs (info, tyName, walk (cutoff + 1) term)
    | TmTyApp (info, term, ty) ->
        TmTyApp (info, walk cutoff term, ontype cutoff ty)
    | TmPack (info, ty1, term, ty2) ->
        TmPack (info, ontype cutoff ty1, walk cutoff term, ontype cutoff ty2)
    | TmUnpack (info, n1, n2, t1, t2) ->
        (* +2 because n1, n2 are introduced in the "in" clause (t2) *)
        TmUnpack (info, n1, n2, walk cutoff t1, walk (cutoff + 2) t2)
  in
  walk cutoff term
...
let typeShiftAbove d cutoff ty =
  tyMap
    (fun cutoff name ctxlen ->
      ...
      if name >= cutoff then
        if name + d >= 0 then TyVar (name + d, ctxlen + d)
        else
          (* Sometimes we have to do a negative shift to leave a scope, but if
             doing such a shift produces negative indices (i.e. we are referencing
             something only visible in the current scope), we should error. *)
          error dummyinfo
            "Scoping error!\n\tType is not visible in current (outer) context."
...
(** Applies a type substitution of s into t; [tyJ->s]t.
    For example, useful in type application terms, like
      (lambda X. term) [T]
    which corresponds to [X->T]term.
    See [ termSubst ] for the logic used here. *)
let typeTermSubst tyJ s term =
  tmMap
    (* Just identity; no need to substitute terms themselves. *)
      (fun info _ name ctxlen -> TmVar (info, name, ctxlen))
    (* Substitute s for tyJ where needed in the type.
       Since we already walked "cutoff" abstractions in, raise the bound
       variable to be replaced correspondingly by that amount. *)
      (fun cutoff tyT -> typeSubst (tyJ + cutoff) s tyT)
    0 term

let typeTermSubstTop tySub term =
  (*
    1. Shift type-to-sub-in term up by 1 to account for the bound variable
       we're going to replace it with (substituting in context with 1 more
       bound variable, namely the one we're replacing).
    2. Perform substitution on first bound type term.
    3. Shift resulting term down by one to account for the destroyed bound
       variable we just substituted.
  *)
  termShift (-1) (typeTermSubst 0 (typeShift 1 tySub) term)
...
let rec printtm ctx t =
  ...
  | TmLoc (_, where) ->
      ...
      pr ">"
  | TmTyAbs (_, tyName, term) ->
      let ctx, tyName = freshname ctx tyName in
      pr ("(λ " ^ tyName ^ ". ");
      printtm ctx term;
      pr ")"
  | TmTyApp (_, term, ty) ->
  ...
  | TmPack (_, ty1, term, ty2) ->
      ...
      pr " as ";
      printty ctx ty2
  | TmUnpack (_, n1, n2, t1, t2) ->
      let ctx, n1 = freshname ctx n1 in
      let ctx, n2 = freshname ctx n2 in
      pr ("let {" ^ n1 ^ ", " ^ n2 ^ "} = ");
      printtm ctx t1;
      pr " in ";
      printtm ctx t2
