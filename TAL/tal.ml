...
let rec ftv_ty =
  ...
  | TTup ts -> List.fold_left union empty (List.map (fun (t, _) -> ftv_ty t) ts)
...
let ftv_subs subs =
  List.fold_left SSet.union SSet.empty (List.map (fun (_, t) -> ftv_ty t) subs)
...
let rfile_simpl rf =
  let rec go seen = function
    | [] -> []
    | (x, t) :: rest ->
        if List.mem x seen then go seen rest else (x, t) :: go (x :: seen) rest
  in
  go [] rf
...
let rec check_wval hctx tctx = function
  ...
  | WVTyApp (w, t) -> (
      ...
      | t ->
          tyerr
            (sprintf "%s cannot be type-applied because it has type %s" (swv w)
               (sty t)))
...
let rec check_sv hctx tctx rctx = function
  ...
  | SVTyApp (v, t) -> (
      ...
      | t ->
          tyerr
            (sprintf "%s cannot be type-applied because it has type %s" (ssv v)
               (sty t)))
  ...
  | Seq (St (rd, idx, rs), i) -> (
      ...
      | Some (TTup ts) -> (
          ...
          | Some (ti, _) ->
              ...
              let ts' =
                List.mapi
                  (fun idx' (t, f) -> if idx = idx' then (t, Init) else (t, f))
                  ts
              in
