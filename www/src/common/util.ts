...
type ToOptionSet<T extends unknown[]> = T extends [infer E, ...infer R] ?
    [[string, E extends string ? StringOptions : E], ...ToOptionSet<R>] :
    [];

export function shapeBackend<O extends Array<Option>>(
    doit: (program: string, ...options: O) => Result,
    options: ToOptionSet<O>): Pick<Backend, 'do'|'options'> {
  return {do: promisify(doit), options};
}
export function shapeBackendP<O extends Array<Option>>(
