...
module Either = struct
  ...
  let partition_map l =
    let rec go lefts rights = function
      ...
      | Left l :: rest -> go (l :: lefts) rights rest
      | Right r :: rest -> go lefts (r :: rights) rest
    in
    go [] [] l
end
...
let process_one _file (lines, queries) (phase, emit) : compile_result =
  ...
  let solve (symbols, e) = Result.ok (symbols, e) in
  let ir (symbols, e) = Result.ok (symbols, e) in
  let eval (symbols, e) = Result.ok (symbols, e) in
  let elab (_symbols, program) =
    if List.length queries = 0 then Error (ElabErr `NoQueries)
    else
      let open Either in
      let queries_with_types =
      ...
      let one_query (((_, cstart), (_, cend)) as loc) =
        ...
        let prefix =
          ...
          ^ " "
        in
        match List.assoc_opt loc queries_with_types with
      ...
      let rec recreate lineno lines =
        let line_queries =
          List.filter (fun ((l, _), _) -> l = lineno) queries
        in
        match (lines, line_queries) with
        | [], _ -> []
        | l :: rest, [] -> Left l :: recreate (lineno + 1) rest
        | l :: rest, line_queries ->
            let rest = recreate (lineno + 1) rest in
            let query_results = List.map one_query line_queries in
            Left l :: (query_results @ rest)
      in
      let oks, errs = Either.partition_map @@ recreate 0 lines in
      match errs with e :: _ -> Error e | [] -> Ok (unlines oks)
  in

  let ( &> ) a b = Result.map b a in
  let print_parsed (symbols, program) =
    Ast.string_of_program ~width:default_width symbols program
  in
  let print_solved (symbols, program) =
    Ast.string_of_program ~width:default_width symbols program
  in
  let print_evaled (symbols, program) =
    let vm_instructions = Vm.compile_program program in
    let result = Eval.eval program in
    let js_code = Js_compiler.compile_program symbols program in
    Printf.sprintf
      "VM Instructions:\n%s\n\nEvaluation Result:\n%s\n\nJavaScript:\n%s"
      (Vm.string_of_instructions vm_instructions)
  ...
  | Ir, Print -> input |> parse >>= solve >>= ir &> print_solved
